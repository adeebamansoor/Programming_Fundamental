# Programming Fundamentals — Labs (1 to 13)

This repository contains all Programming Fundamentals lab tasks (Lab 1 to Lab 13).  
Each lab includes multiple short programming exercises written in **C++**.

---

## 🧪 Lab Overview
### **Lab 01 – Basic I/O & Variables**
- Input/output  
- Variables  
- Simple arithmetic

  ---

### **Lab 02 – Operators**
- Arithmetic operators  
- Relational operators  
- Logical operators

 ---

### **Lab 03 –Comments & Data Types****
- Single-line (`//`) and multi-line (`/* */`) comments  
- Data types: `int`, `float`, `char`, `double`, `bool`  
- Program structure and readability

  ---

### **Lab 04 –Ternary Operator Tasks**
- Understand and apply the ternary operator in real scenarios.  
- Improve logical thinking and C++ coding skills.  

 ---

### **Lab 05 – Decision Making in C++ (if, if-else, )**

-This lab focuses on practicing C++ decision-making statements using:
- `if`
- `if-else`
  
---

### **Lab 06 + 07 – if-elseif-else`, Nested `if`**
-`if-elseif-else`
- Nested `if`
  
---

- ### Lab 08 – Switch Statement
- Use of `switch` statement  
- Menu-driven programs  
- Understanding `case` and `break`  

---

### Lab 09 – While & Do-While Loops
- `while` loop (entry-controlled loop)  
- `do-while` loop (exit-controlled loop)  
- Performing repetitive tasks  

---

### Lab 10 – For Loop
- Structure of `for` loop  
- Iteration control  
- Loop-based problem solving  

---

### Lab 11 – Arrays (Basics)
- Array declaration and initialization  
- Traversing arrays using loops  
- Indexing and size of arrays  

---

### Lab 12 – Arrays (Operations)
- Searching elements in arrays  
- Calculating sum, average, maximum, and minimum  
- Applying arrays in problem-solving  

---

### Lab 13 – Pointers
- Concept of pointers  
- Dereferencing operator (`*`)  
- Types of pointers  
- Pointer arithmetic  
- Using array name as a pointer  

---


## 🎯 Objective
To practice the fundamentals of programming using C++ through small task-based labs .

**How to Run**
1. Open the desired task file in Dev-C++ or any C++ IDE.  
2. Compile and run the program.  
3. Follow the prompts to see output.
---

# 🛠 Tools / Editor Used
- **Dev-C++**  
 

---

## 👤 Author
**Adeeba Mansoor Qureshi**  
Programming Fundamentals – Lab Tasks  

